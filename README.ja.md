...
- デバイスがMacから離れる/近づくと音楽や動画の再生を一時停止/再生します (オプション)
...
BLEUnlockはシステムの再生・一時停止コントロール (Media Remoteフレームワーク) に対応したアプリをロック時に一時停止できます。
対応アプリにはApple Music, QuickTime Player, Spotify, Safari上のYouTubeなどがあります。
残念ながら、今のところGoogle Chromeは一時停止・再生コマンドに反応しないようです。

アプリが「再生中」ウィジェット、もしくはキーボードの ⏯️ キーに対応していれば、BLEUnlockでも制御できるはずです。

### スクリーンセーバーによるロック

「スクリーンセーバーでロック」オプションを使用する場合、正常にロックするには、システム環境設定の *セキュリティとプライバシー* で *スリープとスクリーンセーバの解除にパスワードを要求* を **すぐに** に設定してください。
